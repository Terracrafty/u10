...
export async function findOrCreateTagsFromString(tagString: string) {
  ...
  const tags = splitTagStrings.map((tag) => {
    return prisma.tag.findFirst({
      where: {
        OR: [{ name: { equals: tag } }, { aliases: { has: tag } }],
      },
      select: {
        name: true,
      },
  ...
  const resolvedUniqueTags = [...new Set(await Promise.all(tags))].filter(
    (i) => i !== null
  );
  const uniqueTagNames = resolvedUniqueTags.map((i) => i?.name);
  const uncreatedTagNames = splitTagStrings.filter(
    (i) => !uniqueTagNames.includes(i)
  );
...
export async function searchPosts(
  ...
  includeTags: string[] = [],
  excludeTags: string[] = [],
  olderThan: string = "now",
  newerThan: string = "1970-01-01"
) {
...
postRouter.get("/search", async (req: Request, res: Response) => {
  try {
    const params = req.query as searchPostsParams;
    const {
      authorName,
      titleContains,
      bodyContains,
...
export async function mergeTags(
  ...
  const mergeTo = await prisma.tag.findUniqueOrThrow({
    where: { name: mergeToName },
  });
  const mergeFrom = await prisma.tag.findUniqueOrThrow({
    where: { name: mergeFromName },
  });
